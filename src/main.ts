...
let util = require('./utils/util.ts');
...
module.exports = {
            ...
            if (!rawPassword || !passwordLength) {
                ...
                try {
                    if (!(passwordLength === rawPassword.length && passwordLength >= 12)) {
                        ...
                        await fetch(allConstants.getRemotePassFile())
    ...
    strongPasswordGenerator(userPasswordLength) {
        ...
        let passwordLength = util.inputValidator(userPasswordLength)
        let uCaseLength = 0;
        let lCaseLength = 0;
        let nCaseLength = 0;
        let cCaseLength = 0;
    ...
    completePasswordGeneration(userPasswordLength, autoGeneratedPassword, obfuscatedPassword) {
        let scrambledPassword = util.stringCaster(obfuscatedPassword);
        let passwordLength = util.inputValidator(userPasswordLength)
    ...
    async createStrongPassword(passcodeLength) {
        let codeLength = util.inputValidator(passcodeLength)
    ...
    productKeyGenerator(rawProductKeyLength) {
        let productKeyLength = util.inputValidator(rawProductKeyLength);
